					...
					<div class="col-12  align-self-center">
						<div class="sub-header mt-3 py-3 px-3 align-self-center d-sm-flex w-100 rounded">
							<div class="w-sm-100 mr-auto">
								<h4 class="mb-0">Pending Task Report</h4>
							</div>
							<ol class="breadcrumb bg-transparent align-self-center m-0 p-0">
								<li class="breadcrumb-item">Home</li>
								<li class="breadcrumb-item">Task</li>
								<li class="breadcrumb-item active"><a href="#">Pending Task Report</a></li>
							</ol>
						</div>
					</div>
				</div>
										...
										<tbody>
											<?php
                                                ...
                                                //    $sql = "SELECT * from task  WHERE status = 0";
                                                    $sql = "select * from task as t join task_status as ts on t.id=ts.task_id where ts.id in (select max(id) as max from task_status group by task_id) and ts.status!='Completed' and ts.status!='Cancelled'";
                                                ...
                                                else
                                                {
                                                    ...
                                                    // $sql = "SELECT * from task  WHERE  assigned_to = '$sql_rs_arr2[user_id]'";
                                                    $sql = "SELECT * from task as t join task_status as ts on t.id = ts.task_id WHERE t.assigned_to = '$sql_rs_arr2[user_id]' AND t.end_date<'$date' AND (ts.status='Assigned' OR ts.status = 'In-Process' OR ts.status = 'On Hold')";
                                                }
                                                ...
                                                while($sql_rs_arr=mysqli_fetch_array($sql_rs))
                                                {
													...
													$sql_fetch = "select * from task_status where task_id = '$sql_rs_arr[task_id]' order by id desc limit 1";
													$sql_fetch_rs = mysqli_query($con, $sql_fetch);
													$sql_fetch_arr = mysqli_fetch_array($sql_fetch_rs);
                                                    ?>
                                                    <tr style = "<?php if($sql_rs_arr['end_date']<date('Y-m-d')){ echo "color:red"; } ?>">
														<td><?php echo $i++; ?></td>
